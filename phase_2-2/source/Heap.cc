...
// order maker created based on sorting order maker and cnf query
...
Record* pointerToLiteral;// used in binarySearchCompare to determine which of the arguments is the literal. This changes everytime the function is called because of the way STL binary_search is written.
...
void Heap :: WritePageIfDirty(){
	if(PAGEDIRTY){ // page is dirty. Write it to disk first!
			...
			PAGEDIRTY = false; // reset dirty flag
		}
}
...
int Heap :: GetNext (Record &fetchme) {
	...
	else return 1; // we fetched a record successfully
}
...
void Heap :: Add (Record &addme) {
	if(currPage->Append(&addme) == 0){ // 0 indicates that the last record we wrote
		...
		PAGEDIRTY = false; // reset dirty flag // Added after a2. This was an omission in my a1 submission neeraj
