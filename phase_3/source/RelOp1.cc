...
void GroupBy::StartRun(){
	...
	while(tempOutPipe.Remove(&currRec)){
		if(!prevRec.isNULL()){
			if(comp.Compare(&prevRec, &currRec, groupAtts) != 0){
				//prevRec and currRec are different
				stringstream recSS;
			    Attribute attr;
			    attr.name = "SUM";
			    if (Int == resType){
				...
				int numAttsNow = ((((int*)(prevRec.bits))[1])/sizeof(int)) - 1;
				prevRec.Project(groupAtts->GetWhichAtts(), groupAtts->GetNumAtts(), numAttsNow);
				...
				resRec.MergeRecords(&sumRec, &prevRec, 1, groupAtts->GetNumAtts(), attsToKeep, numAtts, 1);
		...
		ApplyFunction(currRec, intResult, doubleResult, resType);
		prevRec.Consume(&currRec);
...
void GroupBy::PackResultInRecord(Type resType, int &intResult, double &doubleResult, Record &groupRec, Record &resultRec){
	...
	int numAttsNow = ((((int*)(groupRec.bits))[1])/sizeof(int)) - 1;
	groupRec.Project(groupAtts->GetWhichAtts(), groupAtts->GetNumAtts(), numAttsNow);
